...
// ID do workflow preenchido pelo create_workflow_memoria_longa.py após criação
const WORKFLOW_ID = process.env.N8N_MEMORIA_LONGA_WF_ID || ''
...
export async function POST() {
  ...
  if (!WORKFLOW_ID) {
    return NextResponse.json({ error: 'N8N_MEMORIA_LONGA_WF_ID não configurado' }, { status: 503 })
  }
  ...
  try {
    ...
    console.log('[memoria/extrair-longa] disparado:', data?.executionId || 'ok')
