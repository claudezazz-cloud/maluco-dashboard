...
// Retorna conteúdo completo de um POP pelo título (fuzzy match).
export async function GET(req) {
  ...
  // Exact match primeiro, depois ILIKE
  ...
  // Fuzzy: título contém a busca OU busca contém o título
  const fuzzy = await query(
    `SELECT titulo, categoria, conteudo,
       similarity(LOWER(titulo), LOWER($1)) AS sim
     FROM dashboard_pops
     WHERE ativo = true
       AND (LOWER(titulo) ILIKE '%' || LOWER($1) || '%'
            OR LOWER($1) ILIKE '%' || LOWER(titulo) || '%'
            OR similarity(LOWER(titulo), LOWER($1)) > 0.3)
     ORDER BY sim DESC LIMIT 1`,
    [titulo]
  )

  if (fuzzy.rows.length > 0) {
    return NextResponse.json({ found: true, pop: fuzzy.rows[0] })
  }
  ...
  const ts = await query(
    `SELECT titulo, categoria, conteudo FROM dashboard_pops
     WHERE ativo = true
       AND to_tsvector('portuguese', titulo) @@ plainto_tsquery('portuguese', $1)
     ORDER BY ts_rank(to_tsvector('portuguese', titulo), plainto_tsquery('portuguese', $1)) DESC
     LIMIT 1`,
    [titulo]
  )
