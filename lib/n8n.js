...
export async function duplicateWorkflow(baseWorkflowId, newName, webhookPath) {
  ...
  // 5. POST to create — tenta direto primeiro, fallback via webhook interno
  let createRes = await fetch(`${N8N_URL}/api/v1/workflows`, {
    method: 'POST',
    headers: headers(),
    body: JSON.stringify(body),
  })

  // Se bloqueado por Cloudflare (403/503), tenta via localhost
  if (!createRes.ok && (createRes.status === 403 || createRes.status === 503)) {
    createRes = await fetch(`http://localhost:5678/api/v1/workflows`, {
      method: 'POST',
      headers: headers(),
      body: JSON.stringify(body),
    })
